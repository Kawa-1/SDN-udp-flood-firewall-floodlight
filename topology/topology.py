    ...
    def build(self, topology_dict: TopologyDict=topology_dict):
        ...
        for node, node_info in topology_dict.items():
            ...
            if node.startswith('s'):
                ...
                raise ValueError(f"{node} - forbidden to add to this topology other node than switch (s) or host (h)")
                ...
                if link not in created_links:
                    if conn not in nodes:
                        if node.startswith('s'):
                            ...
                            nodes[conn] = self.addHost(conn, ip=topology_dict[conn].get('ip', None))
                        else:
                            raise ValueError(f"{node} - forbidden to add to this topology other node than switch (s) or host (h)")
